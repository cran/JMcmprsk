...
#### Ordinal outcomes
Let $Y_{ij}$ denote the $j$th response measured on subject $i$, where $i=1,\cdots,n$, $j=1,\cdots,n_i$, and $Y_{ij}$ takes values in $\{1,\cdots,K\}$. The competing risks failure times on subject $i$ is $(T_i,D_i)$, and the meaning is the same as in subsection \ref{jmc}.
...
\end{eqnarray*}
where $\pi_{ij}(k)$ stands for the probability that $Y_{ij}\leq k$ given the covariates and the random effects. The implementation of EM algorithm is the same as that of subection \ref{jmc}.
...
\begin{verbatim}sigma_b11, sigma_b12, sigma_b22, sigma_b1u, sigma_b2u 
\end{verbatim}
and so on.
