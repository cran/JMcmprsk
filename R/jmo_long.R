...
jmo_long <- function(long_data, surv_data, out, FE, RE, NP, ID, cate = NULL,
  ...
  ##Construct long_final with ordinal outcome, fixed and random effects for jmo function
  ##Construct m_final for jmo function
  if(!is.null(cate)) {
    for (j in cate){
      dummy_names <- c()
  ...
  ##run jmo function
  res1 <- jmo(p = p, s = s, yfile = long_final, cfile = surv_data, 
